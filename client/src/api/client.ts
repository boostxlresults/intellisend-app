...
export const api = {
  ...
  createCampaign: (tenantId: string, data: { name: string; type?: string; description?: string; segmentId?: string; segmentIds?: string[]; steps?: { bodyTemplate: string; delayMinutes?: number; useAiAssist?: boolean; mediaUrl?: string; sendAsMms?: boolean }[] }) =>
    request<Campaign>(`${API_BASE}/tenants/${tenantId}/campaigns`, {
      method: 'POST',
      body: JSON.stringify(data),
    }),
  ...
  startConversation: async (tenantId: string, contactId: string, message: string, mediaUrl?: string): Promise<{ conversationId: string }> => {
    ...
    await request<{ message: Message }>(`${API_BASE}/tenants/${tenantId}/conversations/${conversation.id}/messages`, {
      method: 'POST',
      body: JSON.stringify({ body: message, mediaUrl }),
    });
